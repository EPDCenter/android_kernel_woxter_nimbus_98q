...
#else
#define BATT_MAX_VOL_VALUE                              8277              	//Full charge voltage
...
static int batt_table[2*BATT_NUM+6] =
{
	...
	6800, 7242, 7332, 7404, 7470, 7520, 7610, 7744, 7848, 8016, 8284,//discharge unused
		...
		if (capacity > bat->bat_capacity){
			if(capacity > bat->bat_capacity + 10 )
			        ...
			        timer_of_charge_sample = NUM_CHARGE_MIN_SAMPLE -5; //10s
			                ...
			                timer_of_charge_sample = NUM_CHARGE_MIN_SAMPLE - 2; // 13
			if (++(bat->gBatCapacityacChargeCnt) >= 3*timer_of_charge_sample){
			...
			bat->gBatCapacityChargeCnt = 0;
		            ...
		            (bat->gBatCapacityChargeCnt)++;
           			if( is_charge_ok(bat) != INVALID_CHARGE_CHECK){
					if( is_charge_ok(bat) == CHARGE_IS_OK){
						if (bat->gBatCapacityChargeCnt >= 3*timer_of_charge_sample){
							bat->gBatCapacityChargeCnt = 0;
							if (bat->bat_capacity <= 99){
								bat->bat_capacity++;
								bat->bat_change  = 1;
...
// printk("xuj, bat ->bat_status = %d, II:FULL = %d, II:NOT_CHARGING = %d, old_capacity = %d, new_capacity = %d, bat_capacity = %d, %s %d \n", 
			// bat ->bat_status, POWER_SUPPLY_STATUS_FULL, POWER_SUPPLY_STATUS_NOT_CHARGING, old_capacity, new_capacity, bat ->bat_capacity, __func__, __LINE__);
	if (bat ->bat_status == POWER_SUPPLY_STATUS_FULL){
		if (new_capacity > 80){
			bat ->bat_capacity = 100;
		}
	...
	//chargeing state
		...
		if( bat  ->pdata->is_reboot_charging == 1)
			bat ->bat_capacity = (old_capacity < 10) ?(old_capacity+1):old_capacity;
		else
			bat ->bat_capacity = (new_capacity > old_capacity) ? new_capacity : old_capacity;
		//xuj add 针对耗完电开机，选择电量优化 20140115
		if(old_capacity < 3 && new_capacity > 3)
			bat ->bat_capacity = 3;
		//_xuj		
	}else{
...
static int iXuj = 0;
if( ++iXuj >= 3){
	printk("xuj, Status = %d, RealAdcVal = %d, RealVol = %d,gBatVol = %d, gBatCap = %d, RealCapacity = %d, batt_dischargecnt = %d\n,  chargecnt = %d,ac_count = %d, usb_count =%d ,usb_dischargecount =%d\n", 
			bat ->bat_status, bat ->adc_val, rk_adc_voltage(bat, bat ->adc_val), 
			bat ->bat_voltage, bat ->bat_capacity, bat ->capacitytmp, bat ->gBatCapacityDisChargeCnt, bat ->gBatCapacityChargeCnt,
			bat ->gBatCapacityacChargeCnt, bat ->gBatCapacityusbChargeCnt, bat ->gBatCapacityusbdisChargeCnt);
	iXuj = 0;
