...
//ÉùÒô×î´ó
	{RT5633_SPK_OUT_VOL		, 0x8000/*0x8042*//*0x8021*/},//speaker output volume is 0db by default    0x8000 0x8021
	{RT5633_DAC_DIG_VOL		, 0x0000},			//phm add				
...
//	{RT5633_AUXMIXER_CTRL		, 0x3e3e},//"AUX Mixer Control"
	...
	{RT5633_GEN_PUR_CTRL_1		, 0x0C00}, //set spkratio to auto  
	
	{RT5633_PRI_REG_ADD		, 0x004a},
	{RT5633_PRI_REG_DATA		, 0x0fc6},

	
	{RT5633_PRI_REG_ADD , 0x0056},
	{RT5633_PRI_REG_DATA , 0x303f},
//	{RT5633_DAC_CTRL,0x0004},	//302
...
//JD setting	
	...
	//{RT5633_JACK_DET_CTRL           , 0x6b00},
	//{RT5633_JACK_DET_CTRL           , 0x6D00},
};
...
{SPK_FR,{0x1AFB,0xF405,0xC4C1,0x1BBC,0x0765,0x07CA,0x12AF,0xF805,0xE904,0x1C10,0x0000,0x1C8B,0x0000,0xc5e1,0x1afb,0x1d46,0x0000,0x0002,0x0007,0x0000,0x0000,0x0000},0x0003},  //20130509
...
static void spk_work_handler(struct work_struct *work)
{
	...
	if(is_spk && !last_is_spk){
		...
		rt5633_update_eqmode(codec, SPK_FR);		// 20130508  SPK is in use, enable EQ mode of NORMAL.
			...
			struct snd_soc_dai *dai)
{
	...
	int coeff = 0;
 rt5633_write_mask(codec, 0x6e,0x2000,0xa000);// 20130508 
...
static int rt5633_hifi_codec_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		rt5633->master = 1;
//		printk(KERN_DEBUG "11111111enter %s\n", __func__);	
		break;
	case SND_SOC_DAIFMT_CBS_CFS:
		iface |= (0x0001 << 15);
		rt5633->master = 0;
//		printk(KERN_DEBUG "22222222enter %s\n", __func__);	
		break;
	default:
		return -EINVAL;
	}
...
//		printk(KERN_DEBUG "333333333enter %s\n", __func__);	
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		iface |= (0x0001);
//		printk(KERN_DEBUG "444444444enter %s\n", __func__);	
		break;
	case SND_SOC_DAIFMT_DSP_A:
		iface |= (0x0002);
//		printk(KERN_DEBUG "5555555555enter %s\n", __func__);	
		break;
	case SND_SOC_DAIFMT_DSP_B:
		iface  |= (0x0003);
//		printk(KERN_DEBUG "6666666666enter %s\n", __func__);	
		break;
	default:
		return -EINVAL;
	}
...
//		printk(KERN_DEBUG "777777777777enter %s\n", __func__);	
		break;
	case SND_SOC_DAIFMT_IB_NF:
		iface |= (0x0001 << 7);
//		printk(KERN_DEBUG "888888888888enter %s\n", __func__);	
		break;
	default:
		return -EINVAL;	
	}
...
static int rt5633_set_bias_level(struct snd_soc_codec *codec, enum snd_soc_bias_level level)
{
	...
	rt5633_write_mask(codec, RT5633_HP_OUT_VOL, 0x0000, 0x8080);
		...
		rt5633_write_mask(codec, RT5633_PWR_MANAG_ADD4, 0xcc00, 0xcc00);
			break;
	printk("phm SND_SOC_BIAS_PREPARE  \n");	
//		printk("%s_HKW00000000============bias_flag=%d\n",__func__,bias_flag);
	...
	case SND_SOC_BIAS_OFF:
		...
		printk("%s__HKW mute\n",__func__);
		rt5633_write_mask(codec, RT5633_HP_OUT_VOL, 0x8080, 0x8080);	//mute hp volume
		//rt5633_write_mask(codec, RT5633_HP_OUT_VOL, 0x8080, 0x0000);
		mdelay(5);
...
void codec_set_spk(bool on)
{
	...
	printk("codec_set_spk on=%d\n",on);
}
int rt5633_shutdown()
{
	struct snd_soc_codec *codec= rt5633_codec;
	rt5633_set_bias_level(codec, SND_SOC_BIAS_OFF);
	printk("***ENTER rt5633_shutdown()*****:hkw\n");
	return 0;
}
